...
# 默认 CSV 路径
_DEFAULT_CSV = os.path.join(os.path.dirname(os.path.dirname(__file__)), "sample_data", "adult_income.csv")


def load_adult_income_partition(node_id: int, num_nodes: int = 2, batch_size: int = 32, csv_path: str = None):
    """
    加载并分区收入预测数据集（从CSV文件）
    ...
    print(f"[节点 {node_id}] 收入预测数据集 | 训练: {len(X_train)} | 测试: {len(X_test)}")

"""
糖尿病预测数据集加载与分区
从 CSV 文件加载数据 (10维特征, 回归任务)，按节点ID进行横向分区
"""
...
# 默认 CSV 路径
_DEFAULT_CSV = os.path.join(os.path.dirname(os.path.dirname(__file__)), "sample_data", "diabetes.csv")


def load_diabetes_partition(node_id: int, num_nodes: int = 2, batch_size: int = 32, csv_path: str = None):
    """
    加载并分区糖尿病数据集（从CSV文件，回归任务）
    ...
    print(f"[节点 {node_id}] 糖尿病数据集(回归) | 训练: {len(X_train)} | 测试: {len(X_test)}")

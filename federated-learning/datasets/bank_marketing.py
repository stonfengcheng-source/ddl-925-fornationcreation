"""
UCI Bank Marketing 银行营销数据集加载与分区
...
# 默认 CSV 路径
_DEFAULT_CSV = os.path.join(os.path.dirname(os.path.dirname(__file__)), "sample_data", "bank_marketing.csv")


def load_bank_marketing_partition(node_id: int, num_nodes: int = 2, batch_size: int = 32, csv_path: str = None):
    """
    加载并分区银行营销数据集（从CSV文件）
    ...
    print(f"[节点 {node_id}] 银行营销数据集 | 训练: {len(X_train)} | 测试: {len(X_test)}")

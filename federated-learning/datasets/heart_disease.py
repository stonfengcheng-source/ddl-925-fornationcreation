...
# 默认 CSV 路径
_DEFAULT_CSV = os.path.join(os.path.dirname(os.path.dirname(__file__)), "sample_data", "heart_disease.csv")


def load_heart_disease_partition(node_id: int, num_nodes: int = 2, batch_size: int = 32, csv_path: str = None):
    """
    加载并分区心脏病数据集（从CSV文件）
    ...
    print(f"[节点 {node_id}] 心脏病数据集 | 训练: {len(X_train)} | 测试: {len(X_test)}")
